...
function wait_for_service {
    SERVICE_NAME=$1
    while true; do
        REPLICAS=$(docker service ls --filter "name=${SERVICE_NAME}" | awk '{print $4}')
...
LOGSTASH_IMG_TAG="5.3.2"
LOGSTASH_SERVICE_NAME="kibana"
...
if [ $serviceFound != 0 ]; then
  ...
  docker service create \
    ...
    --network $network \
    --label com.group="$LABEL_GROUP" \
    --mount type=volume,source=/logstash/logstash.conf,target=/usr/share/logstash/pipeline/logstash.conf \
